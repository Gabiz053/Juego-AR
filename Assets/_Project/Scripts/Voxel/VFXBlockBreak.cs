// ──────────────────────────────────────────────
//  VFXBlockBreak.cs  ·  _Project.Scripts.Voxel
// ──────────────────────────────────────────────
...
namespace _Project.Scripts.Voxel
{
    ...
    [AddComponentMenu("ARmonia/Voxel/VFX Block Break")]
    public class VFXBlockBreak : MonoBehaviour
    {
        #region Inspector ─────────────────────────────────────

        [Tooltip("Particle System child — small block fragments burst.")]
        [SerializeField] private ParticleSystem _particles;

        [Tooltip("Seconds before self-destruction.")]
        [SerializeField] private float _lifetime = 1.0f;

        [Tooltip("Colour of the fragments — set to match the block broken.")]
        ...
        private void Configure()
        {
            ...
            // ── Rotation over lifetime — subtle tumble ─────────
        ...
        {
            if (_particles == null)
                Debug.LogWarning("[VFXBlockBreak] _particles not assigned.", this);
        }
    }
}
