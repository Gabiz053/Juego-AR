...
//  ScriptableObject — all harmony scoring weights and thresholds.
//  Tweak from the Inspector without touching code.
// ??????????????????????????????????????????????
...
namespace _Project.Scripts.Core
{
    ...
    public class HarmonyConfig : ScriptableObject
    {
        ...
        [Tooltip("Minimum number of Sand blocks required.\n" +
                 "Until this is met, the final score is scaled down proportionally.")]
        ...
        [Tooltip("How much the gate suppresses the score when minimums are not met.\n" +
                 "1.0 = score is completely blocked at 0 until minimums are met.\n" +
